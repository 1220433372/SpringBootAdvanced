...
import com.sunxs.boot.config.AdvancedConfig;
import com.sunxs.boot.framework.annotation.Excel;
import com.sunxs.boot.framework.annotation.Excels;
import com.sunxs.boot.framework.response.Result;
import com.sunxs.boot.framework.util.file.FileTypeUtils;
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(Excel.class)){
                    ...
                    if (annotation.isIndex()) {
                        ReflectUtil.setFieldValue(t,field.getName(),i + 1);
                    }
                }
            }
        }
            ...
            for (int i = titleNum + 1; i <= rows; i++)
            {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet())
                {
                    ...
                    else if (Date.class == fieldType)
                    {
                        ...
                        {
                            val = DateUtil.parseDateTime(Convert.toStr(val));
                        }
...
//                            val = reverseDictByExp(Convert.toStr(val), attr.dictType(), attr.separator());
                        ...
                        else if (Excel.ColumnType.IMAGE == attr.cellType() && BeanUtil.isNotEmpty(pictures))
                        {
                            ...
                            else
                            {
                                ...
                                val = FileUtil.writeBytes(data, AdvancedConfig.getImportPath());
                            }
                        }
    ...
    public void fillExcelData(int index, Row row)
    {
        ...
        for (int i = startNo; i < endNo; i++)
        {
            ...
            for (Object[] os : fields)
            {
                Field field = (Field) os[0];
                Excel excel = (Excel) os[1];
                if (Collection.class.isAssignableFrom(field.getType()) && BeanUtil.isNotEmpty(subList))
                {
     ...
     * @param cell 单元格信息
     */
    public void setCellVo(Object value, Excel attr, Cell cell) throws FileNotFoundException {
        if (Excel.ColumnType.STRING == attr.cellType())
        {
            ...
            cell.setCellValue(StrUtil.isBlank(cellValue) ? attr.defaultValue() : cellValue + attr.suffix());
        }
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column)
    {
        ...
        try
        {
            ...
            if (attr.isExport())
            {
                ...
                else if (StrUtil.isNotBlank(dictType))
                {
...
//    }
    ...
    {
        String downloadPath = AdvancedConfig.getDownloadPath() + filename;
    ...
    {
        if (BeanUtil.isNotEmpty(o) && StrUtil.isNotEmpty(name))
        {
    ...
    public String parseDateToStr(String dateFormat, Object val)
    {
        ...
        {
            str = cn.hutool.core.date.DateUtil.format(Convert.toLocalDateTime(val) ,dateFormat);
        }
    ...
    {
        return BeanUtil.isNotEmpty(subFields) && subFields.size() > 0;
    }
    ...
    {
        return BeanUtil.isNotEmpty(subFields) && subFields.size() > 0 && BeanUtil.isNotEmpty(getListCellValue(vo)) && getListCellValue(vo).size() > 0;
    }
